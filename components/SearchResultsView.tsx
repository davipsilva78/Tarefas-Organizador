    ...
    <div>
        ...
        {totalResults === 0 ? (
            <div className="text-center py-16 bg-white dark:bg-gray-800 rounded-lg shadow-md">
                <p className="text-lg text-gray-600 dark:text-gray-300">Nenhum item corresponde à sua pesquisa.</p>
            ...
            <div className="space-y-8">
                {results.tasks.length > 0 && (
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
                        <h3 className="text-xl font-bold text-gray-800 dark:text-gray-200 mb-4">Tarefas ({results.tasks.length})</h3>
                        <ul className="divide-y divide-gray-200 dark:divide-gray-700">
                            {results.tasks.map(task => (
                                <li key={task.id} onClick={() => onTaskClick(task)} className="py-3 flex items-center justify-between cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                    <div>
                ...
                {results.documents.length > 0 && (
                     <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
                        <h3 className="text-xl font-bold text-gray-800 dark:text-gray-200 mb-4">Documentos ({results.documents.length})</h3>
                        <ul className="divide-y divide-gray-200 dark:divide-gray-700">
                            {results.documents.map(doc => (
                                <li key={doc.id} onClick={() => onDocumentClick(doc)} className="py-3 flex items-center justify-between cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50">
