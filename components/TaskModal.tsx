...
const TaskModal: React.FC<TaskModalProps> = ({ task, users, onClose, onSave }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({
        ...formData,
        reminderSent: task?.dueDate !== formData.dueDate ? false : task?.reminderSent,
    } as Task);
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4">
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl max-h-full overflow-y-auto">
        <div className="flex justify-between items-center p-4 border-b dark:border-gray-700">
          <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-200">{task ? 'Editar Tarefa' : 'Nova Tarefa'}</h3>
        ...
        <form onSubmit={handleSubmit}>
          <div className="p-6 space-y-6">
            <div>
              ...
              <input type="text" name="title" id="title" value={formData.title || ''} onChange={handleInputChange} className="w-full border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200 rounded-md shadow-sm focus:ring-custom-blue focus:border-custom-blue" required />
            </div>
            <div>
              <label htmlFor="description" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Descrição</label>
              <textarea name="description" id="description" value={formData.description || ''} onChange={handleInputChange} rows={3} className="w-full border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200 rounded-md shadow-sm focus:ring-custom-blue focus:border-custom-blue"></textarea>
            </div>
            
            <div>
                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Subtarefas</label>
                <div className="space-y-2">
                    {(formData.subtasks || []).map(st => (
                        <div key={st.id} className="flex items-center justify-between bg-gray-50 dark:bg-gray-700 p-2 rounded-md">
                            <div className="flex items-center">
                                ...
                                <span className={`ml-3 text-sm ${st.completed ? 'line-through text-gray-500' : 'text-gray-800 dark:text-gray-300'}`}>{st.text}</span>
                            </div>
                ...
                <div className="flex items-center mt-2">
                    <input type="text" value={subtaskText} onChange={e => setSubtaskText(e.target.value)} placeholder="Adicionar nova subtarefa" className="flex-grow border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200 rounded-l-md shadow-sm focus:ring-custom-blue focus:border-custom-blue text-sm" />
                    <button type="button" onClick={handleAddSubtask} className="px-4 py-2 bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-200 rounded-r-md hover:bg-gray-300 dark:hover:bg-gray-500 font-semibold text-sm">Adicionar</button>
                </div>
            </div>
            ...
            <div className="border-t dark:border-gray-700 pt-4">
                ...
                <div className="flex items-center">
                    ...
                    <button type="button" onClick={handleGetCurrentLocation} className="ml-2 px-3 py-2 text-sm bg-gray-100 dark:bg-gray-600 dark:text-gray-200 rounded-md hover:bg-gray-200 dark:hover:bg-gray-500">Usar localização atual</button>
                </div>
            </div>
            ...
            <div className="border-t dark:border-gray-700 pt-4">
                ...
                <div className="flex items-start">
                    ...
                    <div className="ml-3 text-sm">
                        <label htmlFor="hasReminder" className="font-medium text-gray-700 dark:text-gray-300">Receber lembrete para esta tarefa</label>
                        {formData.hasReminder && (
                            <div className="mt-2 flex items-center space-x-2 text-gray-600 dark:text-gray-400">
                                <span>Lembrar</span>
                                <input type="number" name="reminderOffset" value={formData.reminderOffset || ''} onChange={handleInputChange} className="w-20 border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200 rounded-md shadow-sm text-sm" min="1"/>
                                <select name="reminderOffsetUnit" value={formData.reminderOffsetUnit} onChange={handleInputChange} className="border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-200 rounded-md shadow-sm text-sm">
