...
const DocumentsView: React.FC<DocumentsViewProps> = ({ documents, users, onOpen, onSave }) => {
    ...
    return (
        <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            <p className="text-gray-600 mb-6">Crie, edite e compartilhe documentos com sua equipe. Todos os arquivos são salvos na nuvem e podem ser editados colaborativamente.</p>
                                ...
                                <td className="px-6 py-4">
                                    <div className="flex items-center -space-x-2">
                                        {doc.sharedWith.map(user => (
                                            <img key={user.id} className="w-8 h-8 rounded-full border-2 border-white" src={user.avatarUrl} alt={user.name} title={user.name} />
                                        ))}
                                    </div>
                                </td>
